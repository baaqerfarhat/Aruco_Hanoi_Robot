...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="lab3",
        description="CDS/ME-235 Lab 3 — ArUco detection, grasping, Tower of Hanoi.",
    )
    parser.add_argument("--version", action="version", version=f"%(prog)s {__version__}")
    parser.add_argument("--gui", action="store_true", help="Open the graphical interface.")
    parser.add_argument("-v", "--verbose", action="store_true", help="Print debug messages on stderr.")
    parser.add_argument(
        "--image", type=Path, default=default_practice_image_path(),
